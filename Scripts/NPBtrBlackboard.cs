...
public class NPBtrBlackboard
{
    ...
    private Dictionary<string, List<Action<Type, object>>> observers = new Dictionary<string, List<Action<Type, object>>>();
    private bool isNotifiyng = false;
    private Dictionary<string, List<Action<Type, object>>> addObservers = new Dictionary<string, List<Action<Type, object>>>();
    private Dictionary<string, List<Action<Type, object>>> removeObservers = new Dictionary<string, List<Action<Type, object>>>();
    private List<Notification> notifications = new List<Notification>();
    private List<Notification> notificationsDispatch = new List<Notification>();
    private NPBtrBlackboard parentBlackboard;
    private HashSet<NPBtrBlackboard> children = new HashSet<NPBtrBlackboard>();

    public NPBtrBlackboard(NPBtrBlackboard parent, NPBtrClock clock)
    {
        this.clock = clock;
        this.parentBlackboard = parent;
    }
    public NPBtrBlackboard(NPBtrClock clock)
    {
    ...
    {
        List<Action<Type, object>> observers = GetObserverList(this.observers, key);
        if (!isNotifiyng)
        {
        ...
        else
        {
            if (!observers.Contains(observer))
            {
                List<Action<Type, object>> addObservers = GetObserverList(this.addObservers, key);
            ...
            List<Action<Type, object>> removeObservers = GetObserverList(this.removeObservers, key);
    ...
    private void NotifiyObservers()
    {
        ...
        foreach(NPBtrBlackboard child in children)
        {
...
//                Debug.Log("1 do not notify for key:" + notification.key + " value: " + notification.value);
            ...
            List<Action<Type, object>> observers = GetObserverList(this.observers, notification.key);
        ...
        {
            foreach (Action<Type, object> action in removeObservers[key])
            {
