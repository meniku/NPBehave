...
public class NPBtrParallel : NPBtrComposite
{
    public enum Policy
    {
        ONE,
    ...
    private Dictionary<NPBtrNode, bool> childrenResults;
    private bool successState;
    private bool childrenAborted;

    public NPBtrParallel(Policy successPolicy, Policy failurePolicy, /*Wait waitForPendingChildrenRule,*/ params NPBtrNode[] children) : base("Parallel", children)
    {
        this.successPolicy = successPolicy;
        this.failurePolicy = failurePolicy;
        // this.waitForPendingChildrenRule = waitForPendingChildrenRule;
        this.childrenCount = children.Length;
        this.childrenResults = new Dictionary<NPBtrNode, bool>(); 
    }
    ...
    protected override void DoStart()
    {
        ...
        succeededCount = 0;
        failedCount = 0;
        foreach (NPBtrNode child in this.Children)
        {
            runningCount++;
            child.Start();
        }
    ...
    protected override void DoChildStopped(NPBtrNode child, bool result)
    {
        ...
        if (allChildrenStarted)
        {
            ...
            else if (!this.childrenAborted)
            {
                ...
                if (childrenAborted)
                {
                    foreach (NPBtrNode currentChild in this.Children)
                    {
    ...
    public override void StopLowerPriorityChildrenForChild(NPBtrNode abortForChild, bool immediateRestart)
    {
        ...
        {
            throw new NPBtrException("On Parallel Nodes all children have the same priority, thus the method does nothing if you pass false to 'immediateRestart'!");            
        }
    }
}
